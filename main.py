...
async def compare(ctx: interactions.CommandContext, doter: interactions.api.models.member.Member, hero: str):
    ...
    await ctx.send(f':thinking: Ща, подумаю и узнаем, кто лучше играет на {hero}…')
...
async def last(ctx: interactions.CommandContext):
    ...
    await ctx.send(
        ...
        f"Игра длилась {last_match.duration / 60} минут(ы)\n"
...
async def hero_winrate(ctx: interactions.CommandContext, name: str):
    ...
    if hero is not None:
        ...
        if dota_player_id is not None:
            await ctx.send(dota.get_player_winrate_on_hero(dota_player_id, hero))
        else:
            await ctx.send("Ало, а ты кто такой чтоб кнопки тут жать?")
    else:
